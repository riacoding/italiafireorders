...
export default function EditPage({ id, items }: EditPageParams) {
  const [squareItems, setSquareItems] = useState<HydratedCatalog[]>(items)
  ...
  const toggle = async (catalogItemId: string) => {
    ...
    if (selected.has(catalogItemId)) {
      ...
      const newMenuItem = await createMenuItem({ menuId, catalogItemId })
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    await saveMenuItemsForMenu(savedMenu.id, Array.from(selected))
